# NASA open data — `nasa_apod` / `nasa_neo` / `nasa_mars_photos`

|  |  |
| --- | --- |
| **Module** | [`src/skills/nasa.rs`](../../src/skills/nasa.rs) |
| **Tools** | `nasa_apod`, `nasa_neo`, `nasa_mars_photos` |
...
## What it does
Three read-only lookups against the public api.nasa.gov endpoints: the Astronomy
Picture of the Day, the near-Earth-object feed for a day, and Mars rover imagery.
They work **keyless** by sending NASA's shared `DEMO_KEY` (a very low rate limit);
...
| --- | --- | --- |
| `nasa_apod` | `date?` | Astronomy Picture of the Day: title, date, media type, image/video URL (HD when available), and the explanation. `date` is `YYYY-MM-DD`; omit for today. |
...
- All three are read-only (no confirmation guard).

## Example uses
- **Today's space picture** — `nasa_apod` (no args) for the current APOD; pass
  `date=1996-06-16` for an older one.
